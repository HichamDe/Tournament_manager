...
class TouranementCreationPage(customtkinter.CTk):
    def __init__(self, container):
        ...
        self.img = customtkinter.CTkImage(dark_image=Image.open(
            "./public/img/tbg.png"), size=(300, 250))

        self.img_bg = customtkinter.CTkLabel(
            self.main, image=self.img, text="")
        self.img_bg.grid(row=0, column=1, sticky="news")

        self.fullName = customtkinter.CTkEntry(
            self.input_frame, placeholder_text="fullName")
        self.title = customtkinter.CTkEntry(
            self.input_frame, placeholder_text="title")
        self.type = customtkinter.CTkOptionMenu(
            self.input_frame, values=["LOCAL", "REGIONAL"])
        self.place = customtkinter.CTkEntry(
            self.input_frame, placeholder_text="place")
        self.date = customtkinter.CTkEntry(
            self.input_frame, placeholder_text="date")
        self.btn1 = customtkinter.CTkButton(
            self.input_frame, text="Ceate", command=self.createNewTournament)
    ...
    def createNewTournament(self):
        ...
        if (fullName == "" or title == "" or type == "" or place == "" or date == ""):
            ...
            Tournament(fullName, title, type, place, date).create()
